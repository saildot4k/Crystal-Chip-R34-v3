...
n2epack TMP\OSDPAYDEV9.BIN TMP\OSDPAY.N2E
IF NOT %errorlevel%==0 GOTO error

:: Build CC1.0 OSDSYS payload loader DEV9.
asm5900 -s:FWFS_TEMP_ADDR=0x00110000 -s:BOOT_DEV=9 -s:CC_USE_EEP=1 -s:CC_FORMAT_10=1 -s:PAYLOAD_TYPE=0 -o:TMP\OSLOAD10DEV9.BIN SRC\OSDLOAD.S
IF NOT %errorlevel%==0 GOTO error

:: Build CC1.1 OSDSYS payload loader DEV9.
asm5900 -s:FWFS_TEMP_ADDR=0x00110000 -s:BOOT_DEV=9 -s:CC_USE_EEP=1 -s:PAYLOAD_TYPE=0 -o:TMP\OSLOAD11DEV9.BIN SRC\OSDLOAD.S
IF NOT %errorlevel%==0 GOTO error

:: Build CC1.2 OSDSYS payload loader DEV9.
